...
for future updates, have each of the child components take in the question and set the question text, and display their template when the question updates if the question type is correct. but there is no need for duplicate components for textarea (short answer and essay) and (truefalse and multiple choice) question types that we were handed from the aura components.
*/
...
export default class LwcQuestion extends LightningElement {
    ...
    answersForComboBox=[];
    comboBoxAnswersProvided={choice1:'Test One', choice2:'Test two', choice3:'Test three'};
    get options() {
        return [
            { label: 'New', value: 'new' },
        ...
        if ( typeof this.question!=='undefined' && this.question){
            if( this.question.Options__c && !(this.displayTextarea||this.displayInput)){
                if(this.displayComboBox && this.question.Correct_Answer_s__c){
                    ...
                    this.answersForComboBox=this.createOptionsArrayFromString(this.question.Correct_Answer_s__c);
                    this.answersForComboBox.push({label:'not matched',value:'not matched'})
                    ...
                    this.question.Options__c.split('||').forEach(opt =>{
                        console.log(opt);
                    ...
                    if(typeof answer==='string' && answer.length){
                       ...
                       for(let i = 0;i<answerOptionList.length;i++){
                            let objKeysComboBoxOptions =Object.keys(this.comboBoxAnswersProvided);
                            if(i<objKeysComboBoxOptions.length){
                                ...
                                this.comboBoxAnswersProvided[objKeysComboBoxOptions[i]]=answerOptionList[i] ;
          ...
          let objLength=Object.keys(obj).length;
            ...
            if (this.displayComboBox ){
                  ...
                  //formatting the combo box string to store for matching
                    ...
                    for(let i = 0;i<Object.keys(this.comboBoxAnswersProvided).length;i++){
                     
                        createAnswerString+=this.comboBoxAnswersProvided[Object.keys(this.comboBoxAnswersProvided)[i]]+'||';
                    }
                    ...
                    // });
                    createAnswerString= createAnswerString.length>1? createAnswerString.slice(0,-2):createAnswerString;
